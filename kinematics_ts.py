...
def forward_kinematics(root_positions, rotations, skeleton, positions={}):
    ...
    offsets = torch.broadcast_to(skeleton.jt_offsets, shape[:-2] + (num_jts, 3))
    end_offsets_ts = {}
    ...
    for jt in range(1, num_jts):
        ...
        global_rotations[:, jt] = rotation_ts.quat_mul_quat(global_rotations[:, par_jt], rotations[:, jt])
        global_positions[:, jt] = global_positions[:, par_jt] + rotation_ts.quat_mul_vec(global_rotations[:, par_jt], posis)
